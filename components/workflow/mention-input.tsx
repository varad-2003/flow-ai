...
const MentionInput = ({
  ...
  return (
    <div className={cn(
        `relativew w-full rounded-md border border-input bg-background text-sm text-foreground`, classname
    )}>
        <MentionsInput 
        value={value}
        ...
        >
            ...
            renderSuggestion={
                (entry, _search, _highlighted, _index, focused) => (
                    <CommandItem 
                    value={entry.display}
                    className={cn("flex justify-between text-sm", focused && "bg-accent text-accent-foreground")}
                    >
        ...
        {showTriggerButton && (
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    ...
                    size='icon-sm'
                    className={cn(
                        "absolute h-6 w-6", multiline
                        ? "right-2" : "right-2 top-1/2 -translate-y-1/2"
                    )}
                    >
